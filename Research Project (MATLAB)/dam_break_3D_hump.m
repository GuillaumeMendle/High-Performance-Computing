...
pausetime=0.1;
c=0.8;
D=0.01; %coefficient of artificial viscosity
narrow=0;
...
for i=1:Nx
    for j=1:Ny
        zbin(j,i)=max(0,0.2-0.01*((x(i)-30)^2+(y(j)-15)^2));
    end
end
...
[hhllc,uhllc,vhllc]=boundaries(hhllc,uhllc,vhllc,t,Nx,Ny);

    if narrow==1

[hhllc,uhllc,vhllc]=boundariesV3(hhllc,uhllc,vhllc,x,y,Nx,Ny);
...
while t<ttotal
    ...
    for j=1:Ny-1
        for i=1:Nx-1
            ...
            if neta(j,i)<neta(j+1,i) && uhllc(j+1,i)==0  && zbin(j+1,i)~=0
                deltaneta=neta(j+1,i)-neta(j,i);
                neta(j+1,i)=neta(j+1,i)-deltaneta;
                zb(j+1,i)=neta(j+1,i);
                uhllc(j,i)=0;
            end
        end
    ...
    for j=1:Ny-1
        for i=1:Nx-1
            if neta(j,i)>neta(j+1,i) && uhllc(j+1,i)==0 && zbin(j+1,i)<zbin(j,i)
                vhllc(j+1,i)=0;
            end
        end
    end
    ...
    for j=2:Ny-1
        for i=2:Nx-1
            ...
            un3(j,i)=real(un3(j,i)-lambda*(G3(j,i)-G3(j-1,i))-g*neta(j,i)*lambda*(zb(j+1,i)-zb(j,i)));
    ...
    for j=1:Ny-1
        for i=1:Nx-1
            if neta(j,i)<neta(j,i+1) && uhllc(j,i+1)==0 && zbin(j,i+1)~=0
                deltaneta=neta(j,i+1)-neta(j,i);
                neta(j,i+1)=neta(j,i+1)-deltaneta;
                zb(j,i+1)=neta(j,i+1);
                uhllc(j,i)=0;
            end
        end
...
if t<=4.50
    for j=1:Ny-1
        for i=1:Nx-1
            if neta(j,i)>neta(j,i+1) && uhllc(j,i+1)==0 && zbin(j,i+1)<zbin(j,i)
                uhllc(j,i)=0;
            end
        end
    end
    
elseif t>=4.50 && t<=5.95
     for j=1:Ny-1
        for i=1:Nx-1
            if neta(j,i)>neta(j,i+1) && uhllc(j,i+1)==0 && zbin(j,i+1)<zbin(j,i)
                uhllc(j,i+2)=0;
    ...
    for j=2:Ny-1
        ...
        for i=2:Nx-1
            ...
            un2(j,i)=real(un2(j,i)-lambda*(F2(j,i)-F2(j,i-1))-g*neta(j,i)*lambda*(zb(j,i+1)-zb(j,i)));
    ...
    zb=zbin;
    neta=hhllc+zbin;
    [hhllc,uhllc,neta]=boundariesV2(hhllc,uhllc,neta,zbin,x,t,Nx,Ny);
    
    hold on
    mesh(x,y,neta)
    mesh(x,y,zbin,'edgecolor', 'k')
...
function [u1,u2,u3,F1,F2,F3,G1,G2,G3]=variablesV2(h,u,v,neta,zb,g)
...
function [h,u,v]=boundaries(h,u,v,t,Nx,Ny)
%=
for i=1:Nx
    h(1,i)=h(2,i);
    h(Ny,i)=h(Ny-1,i);
...
function [hhllc,uhllc,neta]=boundariesV2(hhllc,uhllc,neta,zb,x,t,Nx,Ny)

if t<=10
    for j=2:Ny-1
        for i=2:Nx-1
            if zb(j+1,i)~=0 || zb(j,i+1)~=0 || zb(j+1,i+1)~=0 || zb(j,i)~=0 || zb(j-1,i)~=0 || zb(j,i-1)~=0 || zb(j-1,i-1)~=0
                
                if abs(neta(j,i)-zb(j,i))<0.01
...
function [h,u,v]=boundariesV3(h,u,v,x,y,Nx,Ny)
for i=1:Nx
    for j=1:Ny
        if x(i)>=15.2 && x(i)<=15.200001 
            if y(j)<=12.5 || y(j)>=17.5
